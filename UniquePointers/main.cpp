...
int main()
{
	...
	// std::unique_ptr<Widget> uPtr = std::unique_ptr<Widget>(new Widget(2)); // bad way of creating

	{
		// RAII :) (safe exception)
		std::unique_ptr<Widget> uPtr = std::make_unique<Widget>(2);

		// Widget* widget = new Widget();
		// throw std::exception();
		// delete widget; // this dosent work now. it will not dletele since exepction throw so use  uPtr! Unique pointers help, they automaticly manage it
	}
